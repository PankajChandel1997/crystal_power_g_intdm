...
namespace Gurux.DLMS
{
    ...
    public class GXDLMSClient
    {
        ...
        public void update_blockCipherKey(byte[] blockCipherKey)
        {
            if (Settings.Cipher != null)
            {
                Settings.Cipher.BlockCipherKey = blockCipherKey;
            }
        }

        public void update_authenticationKey(byte[] authenticationKey)
        {
            if (Settings.Cipher != null)
            {
                Settings.Cipher.AuthenticationKey = authenticationKey;
            }
        }
        public void update_HLSKey(byte[] HLSKey)
        {
            
               Settings.Password = HLSKey;
        }
        /// <summary>
        ...
        private byte[][] Write2(object name, object value, DataType type, ObjectType objectType, int index)
        {
            ...
            byte[][] reply;
            GXCommon.SetData(Settings, data, type, value);
            if(((string)name == "0.0.13.0.0.255") && (index == 9))
            {
                GXByteBuffer data2 = new GXByteBuffer();
                data2.Set(data.Data, 1, data.Size - 1);
                data = data2;
            }
            if (UseLogicalNameReferencing)
            {
